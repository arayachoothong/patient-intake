...
import {
  ...
  submitErrorMessage,
  submitErrorStatus,
} from "@/domains/patient-intake/helpers/submit-error.helper";
...
describe("submitErrorMessage", () => {
  it("asks the patient to fix fields only on a validation rejection", () => {
    expect(submitErrorMessage(axiosErrorWithStatus(400))).toBe(
      SUBMIT_ERROR_MESSAGES[ApiErrorCode.Invalid],
    );
  });

  it("explains an expired session instead of blaming the fields", () => {
    expect(submitErrorMessage(axiosErrorWithStatus(404))).toBe(
      SUBMIT_ERROR_MESSAGES[ApiErrorCode.NotFound],
    );
  });

  it("reports an already-submitted intake", () => {
    expect(submitErrorMessage(axiosErrorWithStatus(409))).toBe(
      SUBMIT_ERROR_MESSAGES[ApiErrorCode.Conflict],
    );
  });
